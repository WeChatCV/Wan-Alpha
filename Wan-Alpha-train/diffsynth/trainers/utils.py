...
class DiffusionTrainingModule(torch.nn.Module):
    ...
    def add_lora_to_model(self, model, target_modules, lora_rank, lora_alpha=None):
        ...
        lora_config = LoraConfig(r=lora_rank, lora_alpha=lora_alpha, use_dora=True,target_modules=target_modules)
...
class ModelLogger:
    ...
    def on_save_end(self, accelerator, model, epoch_id, step_id):
        accelerator.wait_for_everyone()
        if accelerator.is_main_process:
            state_dict = accelerator.get_state_dict(model)
            state_dict = accelerator.unwrap_model(model).export_trainable_state_dict(state_dict, remove_prefix=self.remove_prefix_in_ckpt)
            os.makedirs(self.output_path, exist_ok=True)
            path = os.path.join(self.output_path, f"epoch-{epoch_id}-{step_id}.safetensors")
            accelerator.save(state_dict, path, safe_serialization=True)
    ...
    for epoch_id in range(num_epochs):
        for data in tqdm(dataloader):
            with accelerator.accumulate(model):
                ...
                if num%250==0:
                    model_logger.on_save_end(accelerator, model, epoch_id, num)
                scheduler.step()
            if accelerator.is_main_process:
                writer.add_scalar('train/loss', loss.detach(), num)
            num+=1
