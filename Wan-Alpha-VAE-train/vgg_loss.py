...
class VGGLoss(nn.Module):
    ...
    def __init__(self, model='vgg16', layer=8, reduction='mean'):
        super().__init__()
        self.reduction = reduction
    ...
    def forward(self, video_input, video_target):
        video_input = video_input * 0.5 + 0.5
        video_target = video_target * 0.5 + 0.5

        input = rearrange(video_input, "B C T H W-> (B T) C H W")
        target = rearrange(video_target, "B C T H W-> (B T) C H W")
