...
class ModelConfig:
    ...
    def download_if_necessary(self, local_model_path="./models", skip_download=False, use_usp=False):
        if self.path is None:
            ...
            for f in self.path:
                assert os.path.exists(f), f"File {f} not found after downloading."
...
class WanVideoPipeline(BasePipeline):
    ...
    def load_dora(self, module, path, alpha=1):
        loader = GeneralLoRALoader(torch_dtype=self.torch_dtype, device=self.device)
        lora = load_state_dict(path, torch_dtype=self.torch_dtype, device=self.device)
        loader.load(module, lora, alpha=alpha)
    ...
    def training_loss(self, **inputs):
        ...
        learnable_alpha = inputs["learnable_alpha"]
        gauss_mask = inputs["gauss_mask"]

        inputs["noise"] = (1-gauss_mask)*(inputs["noise"] + learnable_alpha) + gauss_mask*inputs["noise"]
        ...
        if loss.dtype == torch.long:
            print("cache_path:", inputs["cache_path"])
            print("learnable_alpha:", learnable_alpha)
...
class WanVideoUnit_NoiseInitializer(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, latent_height, latent_width, num_frames, seed, rand_device, vace_reference_image):
        ...
        if vace_reference_image is not None:
            length += 1
        noise = pipe.generate_noise((1, 16, length, latent_height, latent_width), seed=seed, rand_device=rand_device)
        if vace_reference_image is not None:
...
class WanVideoUnit_LoadCaches(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, cache_path, noise, vace_scale) -> dict:
        ...
        cpu_data = torch.load(cache_path, weights_only=False, map_location="cpu")
        data = {}
