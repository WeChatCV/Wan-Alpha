...
def binarize_alpha(frames, threshold=0.5):
    # frames: (T, H, W)
...
def create_gaussian_mask_from_ellipse(shape, mean, axes, angle,
    ...
    """
    - shape: (H, W)
    - mean: (cx, cy) in pixels (x, y)
    - axes: (major_length, minor_length) full lengths in pixels
    - angle: degrees
    - coverage_x/y
    """
    H, W = shape
...
def fit_geometric_ellipse(binary_mask, orient_threshold=0.9, shape_threshold=0.6, enlarge_factor=1.0, min_axis=2.0):
    """
   (cx, cy), (major, minor), angle(deg) or (None, None, None) 
    """
    ...
    coords = (points - mean) @ eigvecs  # Nx2, col0 = major axis coord
...
def generate_best_aligned_gaussian(binary_mask,
                                   ...
                                   debug=False):
    """
    generate aligned gaussian mask
    """
    ...
    for it in range(max_enlarge_iters):
        ...
        try:
            ...
            if debug:
                print("cv2.ellipse failed with axes", axes_cv, "angle", angle)
            break
        ...
        if debug:
            print(f"[iter {it}] axes={cur_major:.1f},{cur_minor:.1f} cover={cover:.4f}")

        if cover >= coverage_target:
            if debug:
                print("Reached coverage target:", cover)
            break
    ...
    if debug:
        print("final axes:", final_axes, "best_cover:", best_cover)
    return gauss, mean, final_axes, angle


def gen_gauss_mask_for_video(alpha_frames):
    binary_alpha = binarize_alpha(alpha_frames[0][0].to(dtype=torch.float32).cpu().detach().numpy(), threshold=0.5)
    compressed_masks = spatial_temporal_compress(binary_alpha)

    orient_threshold = 0.9   #  angle
    shape_threshold = 0.2    #  axes / mean
    ...
    return torch.tensor(gauss_info)
