...
def _parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="The path to the cache gaussian mask.")
...
def generate(args):
    ...
    for args.prompt in all_prompts:
        ...
        cpu_mask = torch.load(args.cache_path_mask, weights_only=False, map_location="cpu")
        gauss_mask = torch.tensor(cpu_mask).to(wan_t2v.device, dtype=torch.bfloat16)

        videos = wan_t2v.generate_mask(
            args.alpha_shift_mean,
            gauss_mask,
            args.prompt,
