...
class BasePipeline(torch.nn.Module):
    ...
    def check_resize_height_width_latent(self, height, width, num_frames=None):
        # Shape check
        if height % 2 != 0:
            height = (height + 2 - 1) // 2 * 2
            print(f"height % {2} != 0. We round it up to {height}.")
        if width % 2 != 0:
            width = (width + 2 - 1) // 2 * 2
            print(f"width % {2} != 0. We round it up to {width}.")
        if num_frames is None:
            return height, width
        else:
            if num_frames % self.time_division_factor != self.time_division_remainder:
...
class WanVideoPipeline(BasePipeline):
    ...
    def training_loss(self, **inputs):
        ...
        timestep = timestep.to(device=inputs["latents"].device)
        
        noise_pred = self.model_fn(**inputs, timestep=timestep)
        current_device = torch.cuda.current_device()
        training_target = training_target.to(device=current_device)
        diff = noise_pred.float() - training_target.float()  
        diff_squared = diff **2  
        weighted_diff = diff_squared * inputs["rgb_mask"]  
        
        loss = torch.clamp((weighted_diff * self.scheduler.training_weight(timestep)),0,1).mean()
        return loss
    ...
    def __call__(
        ...
        # Decode
        # latent = torch.rand(1,16,20,80,78)
...
class WanVideoUnit_NoiseInitializer(PipelineUnit):
    def __init__(self):
        super().__init__(input_params=("latent_height", "latent_width", "num_frames", "seed", "rand_device", "vace_reference_image"))

    def process(self, pipe: WanVideoPipeline, latent_height, latent_width, num_frames, seed, rand_device, vace_reference_image):
        ...
        noise = pipe.generate_noise((2, 16, length, latent_height, latent_width), seed=seed, rand_device=rand_device)
        if vace_reference_image is not None:
...
class WanVideoUnit_LoadCaches(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params={"cache_path": "cache_path", "job_name": "job_name","noise": "noise", "vace_scale": "vace_scale"},
        )

    def process(self, pipe: WanVideoPipeline, cache_path, job_name,noise, vace_scale) -> dict:
        data_list=[]
        datas = {}
        for i in range(len(cache_path)):
            cache_path[i] = cache_path[i] + "_" + job_name

            cpu_data = torch.load(cache_path[i], weights_only=True, map_location="cpu")
            data = {}
        ...
        # datas["context"] = torch.stack(data_list)
        datas["context"] = data_list[0]
        return data
    ...
    **kwargs,
    ...
    if use_unified_sequence_parallel:
        ...
        from xfuser.core.distributed import (get_sequence_parallel_rank,
                                            ...
                                            get_sp_group)
    # print("timestep.device:",timestep.device)
    # print("latents.device:",latents.device)
    current_device = torch.cuda.current_device()
    timestep = timestep.to(device=current_device)
    t = dit.time_embedding(sinusoidal_embedding_1d(dit.freq_dim, timestep))
    t_mod = dit.time_projection(t).unflatten(1, (6, dit.dim))
    if motion_bucket_id is not None and motion_controller is not None:
        t_mod = t_mod + motion_controller(motion_bucket_id).unflatten(1, (6, dit.dim))
    context = context.to(device=current_device)
    ...
    # print("control_camera_latents_input.device:", control_camera_latents_input.device)
